...
export default function Preferences(props) {
  ...
  const updateLink = useCallback(
    (prefs) => {
      ...
      const latLong = [south, north, west, east]
        .map((x) => x.toFixed(3))
        .join(',');
  ...
  return (
    ...
    (
      <div className="c7e-preferences">
        ...
        <AuthnControl
          container={ref.authnControlDiv}
          map={ref.map}
          isAuthenticated={isAuthenticated}
          onToggleAuthn={onToggleAuthn}
        />
        <RegionMaskControl
          container={ref.zoomControlDiv}
          map={ref.map}
          onSelectRegion={handleSelectRegion}
        />
        <RegionMask
          bounds={bounds}
          container={ref.regionMaskDiv}
          live={ref}
          map={ref.map}
          onResize={handleRegionUpdate}
        />
      </div>
    )
  );
...
const DEGREES_RADIUS_SQ = Math.pow(
  (DegreesOffet.EAST + DegreesOffet.NORTH) / 2,
  2
);
...
function initializeMap(ref) {
  if (ref.map == null) {
    const { L } = window;
    const map = (ref.map = L.map('preferences-map', Props.MAP).setView(
      Props.CENTER,
      6
    ));
